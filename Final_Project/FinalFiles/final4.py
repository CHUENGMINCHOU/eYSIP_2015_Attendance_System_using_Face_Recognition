...
class Ui_MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        QtCore.QTimer.singleShot(100, self.timer_func)
...
"color: rgb(255, 255, 255);"))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.EntryName.setStatusTip(_translate("MainWindow", "Exit Name", None))
    ...
    def timer_func(self):
        ret, frame = self.camera1.read(0)
        cv2.imwrite("image.jpg", frame)
        self.EntryStream.setPixmap(QtGui.QPixmap(_fromUtf8("./image.jpg")))
        if threading.active_count() < 2 :
            thread = RecognizerClass(frame)
            thread.start()
        ...
        self.EntryTime.setText(_translate("MainWindow", "<html><head/><body><p><span style=\" font-size:11pt; font-weight:600; color:#ffffff;\">" + entryDate +"</span></p></body></html>", None))
...
class RecognizerClass(threading.Thread):
    ...
    def run(self):
        global n
        global entryDetection, exitDetection, entryTime, exitTime, entryDate, exitDate
        global entryImage, exitImage
        global failure
        global lastDetected
        label = get_label_2(self.__frame)
        print label_to_name(label)
        print n
        detections[label] = detections[label] + 1
        n = n + 1        
        if n > 20:
            n = 0
            detectedLabel = findMax(detections)
            ...
            elif detectedLabel != lastDetected:
                print label_to_name(detectedLabel), " is detected"
                entryDetection = "Name: " + label_to_name (detectedLabel)
                entryDate = "Entry: " + datetime.datetime.now().strftime("%I:%M %p")
                if detectedLabel < 10:
                    entryImage = "./displayImages/subject0" + str(detectedLabel) + "_smile.jpg"
                else:
                    entryImage = "./displayImages/subject" + str(detectedLabel) + "_smile.jpg"
                
                new_entry(label_to_name (detectedLabel), 10, datetime.datetime.now().strftime("%I:%M %p"))
