...
def label_to_name(label, confidence):
	if label == 1:
			...
			print " Devendra is detected with confidence {}".format(confidence)
	elif label == 8:
			print "Saakshi is detected with confidence {}".format(confidence)
	elif label == 9:
			print "{} is detected with confidence {}".format(label, confidence)
	elif label == 10:
			print "Ramiz is detected with confidence {}".format(confidence)
	elif label == 11:
			...
			print "Suresh is detected with confidence {}".format(confidence)
	elif label == 15:
			print "Uttam is detected with confidence {}".format(confidence)
...
images, labels = get_training_set('./DB_final')

face_recognizer.train(images, np.array(labels))

input_image = cv2.imread(sys.argv[1])
height, width, c = input_image.shape
input_image = cv2.resize(input_image, (width/8, height/8), interpolation = cv2.INTER_CUBIC)
